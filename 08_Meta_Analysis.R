...
# ---- S3: MICE data ----
cat("--------------------------------------------------------------------------------\n")
cat("Processing: MICE Data (from pooled imputed file)\n")
cat("--------------------------------------------------------------------------------\n")
...
if (file.exists(mice_csv_path)) {
  ...
  cat("  Loaded pooled MICE data:", nrow(pooled_mice), "rows\n")
  
  # Process by cohort
  for (coh in unique(pooled_mice$cohort)) {
    df <- pooled_mice[pooled_mice$cohort == coh, ]
    bw <- cohort_info[[coh]]$wave
    score_var <- paste0("w", bw, "_unhealthy_score")
    
    # Check if score variable exists, if not try 'unhealthy_score'
    if (!score_var %in% names(df) && "unhealthy_score" %in% names(df)) {
      score_var <- "unhealthy_score"
    }
    
    if (!score_var %in% names(df)) {
      cat("    ", coh, ": score variable not found, skipping\n")
      next
    }
    ...
    cat("    ", coh, ": Processed (N=", nrow(df), ")\n")
  }
} else {
  cat("  [INFO] MICE imputed data not found. Run 02_MICE_Imputation.R first.\n")
  cat("  Skipping S3 sensitivity analysis for meta-analysis.\n")
}
...
# =============================================================================
# PART 7B: SUBGROUP META-ANALYSIS - by Age and Sex
# =============================================================================

cat("=============================================================================\n")
cat("PART 7B: SUBGROUP META-ANALYSIS\n")
cat("=============================================================================\n\n")

# Note: True subgroup meta-analysis requires study-specific age/sex stratified HRs
# Here we simulate by examining heterogeneity using moderator analysis if available

subgroup_results <- list()

# For each meta-analysis, perform subgroup tests if multiple studies
...
for (key in names(ma_results)) {
  ...
  # Prediction interval as a measure of heterogeneity
  pi_lower <- exp(ma$lower.predict)
  pi_upper <- exp(ma$upper.predict)
  
  subgroup_results[[key]] <- data.frame(
    Analysis = key,
    K_Studies = ma$k,
    I2 = round(ma$I2 * 100, 1),
    I2_CI = sprintf("(%.1f, %.1f)", ma$lower.I2 * 100, ma$upper.I2 * 100),
    tau2 = round(ma$tau2, 4),
    Prediction_Interval = sprintf("%.2f - %.2f", pi_lower, pi_upper),
    Heterogeneity_Interpretation = case_when(
      ma$I2 < 0.25 ~ "Low",
      ma$I2 < 0.50 ~ "Low-Moderate",
      ma$I2 < 0.75 ~ "Moderate-High",
      TRUE ~ "High"
    ),
    Q_pvalue = format.pval(ma$pval.Q, digits = 4),
    stringsAsFactors = FALSE
  )
}
...
cat("Total rows:", nrow(subgroup_df), "\n")

tryCatch({
  heterogeneity_file <- file.path(OUTPUT_DIR, "Meta_Heterogeneity_Summary.csv")
  if (nrow(subgroup_df) > 0) {
    print(subgroup_df %>% select(Analysis, K_Studies, I2, Heterogeneity_Interpretation, Q_pvalue))
    write.csv(subgroup_df, heterogeneity_file, row.names = FALSE)
  } else {
    placeholder <- data.frame(Note = "No heterogeneity results - check if meta-analyses ran successfully")
    write.csv(placeholder, heterogeneity_file, row.names = FALSE)
  }
  cat("\n  [OK] Saved: Meta_Heterogeneity_Summary.csv\n")
}, error = function(e) {
  cat("  [ERROR]", e$message, "\n")
})

# Note for publication-ready subgroup analysis
cat("\n--- Note on Subgroup Meta-Analysis ---\n")
cat("  For publication-quality subgroup analyses (by age, sex, region):\n")
cat("  1. Each cohort needs to provide stratified HRs\n")
cat("  2. This requires re-running Cox models in 04_Pooled_Cox_Analysis.R\n")
cat("  3. Then meta-analyzing the stratified results separately\n")
cat("  Current results show pooled subgroup analyses from Cox_Age_Stratified.csv\n")
cat("  and Cox_Sex_Stratified.csv files.\n\n")
...
results_for_excel <- list(
  ...
  "Heterogeneity_Summary" = if(exists("subgroup_df") && nrow(subgroup_df) > 0) subgroup_df else data.frame(Note = "No data"),
...
cat("\n--- Saving Meta-Analysis Excel and CSV Files ---\n")

# Save Excel with tryCatch
tryCatch({
  cat("  Saving Excel file...\n")
  filepath_xlsx <- file.path(OUTPUT_DIR, "Meta_Analysis_Comprehensive.xlsx")
  writexl::write_xlsx(results_for_excel, filepath_xlsx)
  cat("  [OK] Saved: Meta_Analysis_Comprehensive.xlsx\n")
}, error = function(e) {
...
tryCatch({
  if (nrow(ma_summary_df) > 0) {
    filepath_csv <- file.path(OUTPUT_DIR, "Meta_Analysis_Summary.csv")
    write.csv(ma_summary_df, filepath_csv, row.names = FALSE)
    cat("  [OK] Saved: Meta_Analysis_Summary.csv (", nrow(ma_summary_df), " rows)\n")
  }
}, error = function(e) {
  cat("  [ERROR] Main summary CSV save failed:", e$message, "\n")
})

# ===== Save Individual CSV Files for Each Component =====
cat("\n--- Saving Individual Meta-Analysis CSV Files ---\n")
...
safe_save_csv(study_characteristics, "Meta_Study_Characteristics.csv", "Study Characteristics")

# 2. Study-Specific HR Results
safe_save_csv(study_hr_data, "Meta_Study_Specific_HR.csv", "Study-Specific HR")

# 3. Meta-Analysis Summary by Analysis Type
if (!is.null(ma_summary_df) && nrow(ma_summary_df) > 0) {
  # Primary Analysis Only
  primary_ma <- ma_summary_df %>% filter(Analysis == "Primary")
  safe_save_csv(primary_ma, "Meta_Primary_Summary.csv", "Primary MA")
  ...
  for (sens_analysis in c("S1_5Level", "S2_HeavyDrink", "S3_MICE", "S4_Drop1st")) {
    ...
    if (nrow(sens_ma) > 0) {
      ...
      safe_save_csv(sens_ma, filename, sens_analysis)
    }
  }
}

# 4. Leave-One-Out Results
safe_save_csv(loo_summary_df, "Meta_LeaveOneOut.csv", "Leave-One-Out")

# 5. Egger's Test Results
safe_save_csv(egger_summary_df, "Meta_EggersTest.csv", "Egger's Test")
...
tryCatch({
  if (!is.null(ma_summary_df) && nrow(ma_summary_df) > 0) {
    ...
    filepath <- file.path(OUTPUT_DIR, "Meta_Summary_Publication.csv")
    write.csv(pub_summary, filepath, row.names = FALSE)
    cat("  [OK] Saved: Meta_Summary_Publication.csv (", nrow(pub_summary), " rows)\n")
  }
}, error = function(e) {
  cat("  [ERROR] Publication summary save failed:", e$message, "\n")
})
...
tryCatch({
  ...
  cat("  [OK] Saved: Meta_analysis_objects.rds\n")
}, error = function(e) {
  cat("  [ERROR] Failed to save Meta RDS:", e$message, "\n")
})
...
meta_files <- c(
  ...
  "Meta_S3_MICE_Summary.csv",
  "Meta_S4_Drop1st_Summary.csv",
...
cat("\n  - Forest plots directory:", fig_dir, "\n")
cat("  - Funnel plots directory:", funnel_dir, "\n")
