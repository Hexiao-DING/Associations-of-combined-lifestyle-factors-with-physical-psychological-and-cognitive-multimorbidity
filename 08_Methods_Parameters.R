...
mice_parameters <- data.frame(
  Parameter = c(
    ...
    "Variables to Impute",
    "Predictor Variables",
    "Excluded Variables (high missing)",
    "Total Level 2 Clusters (Countries)"
  ),
  Value = c(
    "2-Level Multiple Imputation by Chained Equations (MICE)",
    MICE_CONFIG$cluster_var,
    "Individual",
    as.character(MICE_CONFIG$m),
    as.character(MICE_CONFIG$maxit),
    as.character(MICE_CONFIG$seed),
    "mice + miceadds",
    "2l.pmm (Multilevel Predictive Mean Matching)",
    paste(MICE_CONFIG$vars_to_impute, collapse = ", "),
    paste(MICE_CONFIG$predictor_vars, collapse = ", "),
    paste(MICE_CONFIG$excluded_vars, collapse = ", "),
    "20"
  ),
  Description = c(
    "Multilevel imputation accounting for country-level clustering",
    "Countries used as Level 2 clustering units",
    "Individual participants within countries",
    ...
    "Variables with >1% missing",
    "Complete variables used to predict missing values",
    "ELSA rural 100% missing, not imputable",
    "20 countries across 5 cohorts"
  ),
  stringsAsFactors = FALSE
)
