...
# IMMEDIATE SAVE: Save S1 results right away
cat("\n--- Immediate Save: S1 5-Level Results ---\n")
tryCatch({
  if (nrow(sens1_results_df) > 0) {
    write.csv(sens1_results_df, file.path(OUTPUT_DIR, "Cox_S1_5Level_Categories.csv"), row.names = FALSE)
    cat("  [OK] Saved Cox_S1_5Level_Categories.csv (", nrow(sens1_results_df), " rows)\n")
  } else {
    cat("  [WARN] No S1 results to save\n")
  }
}, error = function(e) {
  cat("  [ERROR] Failed to save S1 results:", e$message, "\n")
})
...
# IMMEDIATE SAVE: Save S2 results right away
cat("\n--- Immediate Save: S2 Heavy Drinking Results ---\n")
tryCatch({
  if (nrow(sens2a_results_df) > 0) {
    write.csv(sens2a_results_df, file.path(OUTPUT_DIR, "Cox_S2a_HeavyDrink_Individual.csv"), row.names = FALSE)
    cat("  [OK] Saved Cox_S2a_HeavyDrink_Individual.csv (", nrow(sens2a_results_df), " rows)\n")
  } else {
    cat("  [WARN] No S2a results to save\n")
  }
}, error = function(e) {
  cat("  [ERROR] Failed to save S2a results:", e$message, "\n")
})

tryCatch({
  if (nrow(sens2b_results_df) > 0) {
    write.csv(sens2b_results_df, file.path(OUTPUT_DIR, "Cox_S2b_HeavyDrink_Cumulative.csv"), row.names = FALSE)
    cat("  [OK] Saved Cox_S2b_HeavyDrink_Cumulative.csv (", nrow(sens2b_results_df), " rows)\n")
  } else {
    cat("  [WARN] No S2b results to save\n")
  }
}, error = function(e) {
  cat("  [ERROR] Failed to save S2b results:", e$message, "\n")
})
...
if (file.exists(mice_csv_path)) {
  ...
  cat("  Loaded MICE data:", nrow(pooled_mice), "rows\n")
  cat("  MICE columns:", paste(names(pooled_mice), collapse = ", "), "\n")
  
  # Check if main data (pooled_analysis) has lifestyle variables
  cat("\n  Checking main data for lifestyle variables...\n")
  cat("  Main data rows:", nrow(pooled_analysis), "\n")
  
  # MICE data has imputed 'edu', we need to merge with lifestyle variables from main data
  # Strategy: Use row-by-row matching since both datasets have same structure
  
  if (nrow(pooled_mice) == nrow(pooled_main)) {
    cat("  [OK] MICE and main data have same number of rows - can merge by position\n")
    
    # Add lifestyle variables from main data to MICE data
    lifestyle_vars_to_add <- c("unhealthy_drink", "unhealthy_smoke", "unhealthy_pa", "unhealthy_soc",
                                "heavy_drink", "unhealthy_score_heavy",
    ...
    for (var in lifestyle_vars_to_add) {
      if (var %in% names(pooled_main) && !(var %in% names(pooled_mice))) {
        pooled_mice[[var]] <- pooled_main[[var]]
        cat("    Added:", var, "\n")
      }
    }
    
    cat("  Updated MICE columns:", length(names(pooled_mice)), "\n")
    
  } else {
    cat("  [WARN] Row count mismatch - cannot merge by position\n")
    cat("  MICE rows:", nrow(pooled_mice), ", Main rows:", nrow(pooled_main), "\n")
  }
  
  # Check if outcome variables exist
  if ("event_ppcmm" %in% names(pooled_mice) && "time_ppcmm_months" %in% names(pooled_mice)) {
    
    # Prepare MICE analysis data with full variables
    cat("\n  Preparing MICE analysis data...\n")
    
    # Check if lifestyle variables are available
    has_lifestyle_vars <- all(c("unhealthy_drink", "unhealthy_smoke", "unhealthy_pa", "unhealthy_soc") %in% names(pooled_mice))
    cat("  Has individual lifestyle variables:", has_lifestyle_vars, "\n")
    
    if (has_lifestyle_vars) {
      # Full preparation with lifestyle variables
      pooled_mice_analysis <- pooled_mice %>%
        filter(!is.na(age_baseline),
               !is.na(sex),
               !is.na(edu),
               !is.na(unhealthy_score),
               !is.na(unhealthy_drink),
               !is.na(unhealthy_smoke),
               !is.na(unhealthy_pa),
               !is.na(unhealthy_soc),
               !is.na(event_ppcmm),
               !is.na(time_ppcmm_months),
               time_ppcmm_months > 0) %>%
        mutate(
          sex = relevel(factor(sex), ref = "Men"),
          edu = relevel(factor(edu), ref = "Primary or below"),
      ...
      # Simplified preparation without lifestyle variables
      pooled_mice_analysis <- pooled_mice %>%
        filter(!is.na(age_baseline),
               !is.na(sex),
               !is.na(edu),
    ...
    if (nrow(pooled_mice_analysis) > 100) {
      ...
      # Use covariates
      mice_covariates <- c("age_baseline", "sex", "edu", "region")
      mice_covariates <- mice_covariates[mice_covariates %in% names(pooled_mice_analysis)]
      cat("  Using covariates:", paste(mice_covariates, collapse = ", "), "\n")
      
      # ========== S3 Individual Factors (if lifestyle variables available) ==========
      if (has_lifestyle_vars) {
        cat("\n--- S3: Individual Factors (MICE) ---\n")
        
        for (out_name in all_outcomes) {
          out_info <- outcome_info[[out_name]]
          event_var <- out_info$event
          time_var <- out_info$time
          
          # Check if outcome variables exist
          if (!event_var %in% names(pooled_mice_analysis) || !time_var %in% names(pooled_mice_analysis)) {
            cat("    [SKIP]", out_name, "- outcome variables not found\n")
            next
          }
          
          df_mice <- pooled_mice_analysis %>%
            filter(!is.na(.data[[event_var]]) & !is.na(.data[[time_var]]) & .data[[time_var]] > 0)
          
          n_total <- nrow(df_mice)
          n_events <- sum(df_mice[[event_var]], na.rm = TRUE)
          
          cat("  ", out_name, ": N =", n_total, ", Events =", n_events, "\n")
          ...
          formula_str <- paste0("Surv(", time_var, ", ", event_var, ") ~ ",
                                ...
                                paste(mice_covariates, collapse = " + "), " + strata(cohort)")
          
          fit <- tryCatch({
            coxph(as.formula(formula_str), data = df_mice)
          }, error = function(e) {
            cat("    [ERROR]", e$message, "\n")
            NULL
          })
          ...
          if (!is.null(fit)) {
            ...
            if (sum(exp_rows) > 0) {
              sens3_individual[[out_name]] <- data.frame(
                Model = "S3: MICE - Individual Factors",
                Outcome = out_name,
                Outcome_Label = out_info$label,
                Outcome_Type = out_info$type,
                Variable = rownames(s$coefficients)[exp_rows],
                Variable_Label = var_labels[rownames(s$coefficients)[exp_rows]],
        ...
        cat("\n--- S3: Individual Factors (MICE) - SKIPPED (no lifestyle variables) ---\n")
      }
      
      # ========== S3 Cumulative Effect ==========
      ...
      for (out_name in all_outcomes) {
        ...
        # Build formula
        formula_str <- paste0("Surv(", time_var, ", ", event_var, ") ~ n_lifestyle_cat + ",
                              paste(mice_covariates, collapse = " + "), " + strata(cohort)")
        ...
        if (!is.null(fit)) {
          ...
          if (sum(exp_rows) > 0) {
            # Calculate P for trend
            trend_formula <- paste0("Surv(", time_var, ", ", event_var, ") ~ n_lifestyle_continuous + ",
                                    paste(mice_covariates, collapse = " + "), " + strata(cohort)")
            trend_fit <- tryCatch({
              coxph(as.formula(trend_formula), data = df_mice)
            }, error = function(e) NULL)
            
            p_trend <- if (!is.null(trend_fit)) {
              format.pval(summary(trend_fit)$coefficients["n_lifestyle_continuous", "Pr(>|z|)"], digits = 3)
            } else {
              NA
            }
            
            sens3_cumulative[[out_name]] <- data.frame(
              Model = "S3: MICE - Cumulative 4-level",
              Outcome = out_name,
              Outcome_Label = out_info$label,
              Outcome_Type = out_info$type,
              Variable = rownames(s$coefficients)[exp_rows],
...
# IMMEDIATE SAVE: Save S3 results right away
cat("\n--- Immediate Save: S3 MICE Results ---\n")
tryCatch({
  if (nrow(sens3_individual_df) > 0) {
    write.csv(sens3_individual_df, file.path(OUTPUT_DIR, "Cox_S3_MICE_Individual.csv"), row.names = FALSE)
    cat("  [OK] Saved Cox_S3_MICE_Individual.csv (", nrow(sens3_individual_df), " rows)\n")
  } else {
    cat("  [WARN] No S3 individual results to save\n")
  }
}, error = function(e) {
  cat("  [ERROR] Failed to save S3 individual results:", e$message, "\n")
})

tryCatch({
  if (nrow(sens3_cumulative_df) > 0) {
    write.csv(sens3_cumulative_df, file.path(OUTPUT_DIR, "Cox_S3_MICE_Cumulative.csv"), row.names = FALSE)
    cat("  [OK] Saved Cox_S3_MICE_Cumulative.csv (", nrow(sens3_cumulative_df), " rows)\n")
  } else {
    cat("  [WARN] No S3 cumulative results to save\n")
  }
}, error = function(e) {
  cat("  [ERROR] Failed to save S3 cumulative results:", e$message, "\n")
})
...
if ("event_ppcmm_drop1st" %in% names(pooled_analysis)) {
  ...
  if (n_events_drop1st >= 10) {
    # Individual factors (with strata(cohort) for pooled analysis)
    formula_str <- paste0("Surv(time_ppcmm_drop1st, event_ppcmm_drop1st) ~ ",
                          "unhealthy_drink + unhealthy_smoke + unhealthy_pa + unhealthy_soc + ",
                          paste(covariates, collapse = " + "), " + strata(cohort)")
    
    cat("  Formula (Individual):", formula_str, "\n")
    ...
    # Cumulative effect (with strata(cohort) for pooled analysis)
    formula_str2 <- paste0("Surv(time_ppcmm_drop1st, event_ppcmm_drop1st) ~ ",
                           "n_lifestyle_cat + ", paste(covariates, collapse = " + "), " + strata(cohort)")
    
    cat("  Formula (Cumulative):", formula_str2, "\n")
    ...
    if (!is.null(fit2)) {
      ...
      # P for trend (also with strata)
      fit_trend <- tryCatch({
        coxph(as.formula(paste0("Surv(time_ppcmm_drop1st, event_ppcmm_drop1st) ~ ",
                                "n_lifestyle_continuous + ", paste(covariates, collapse = " + "), " + strata(cohort)")),
              data = df_drop1st)
      }, error = function(e) NULL)
      
      p_trend <- if (!is.null(fit_trend)) {
        format.pval(summary(fit_trend)$coefficients["n_lifestyle_continuous", "Pr(>|z|)"], digits = 3)
      } else {
        NA
      }
...
# IMMEDIATE SAVE: Save S4 results right away
cat("\n--- Immediate Save: S4 Drop1st Results ---\n")
tryCatch({
  if (nrow(sens4_results_df) > 0) {
    write.csv(sens4_results_df, file.path(OUTPUT_DIR, "Cox_S4_Drop1st.csv"), row.names = FALSE)
    cat("  [OK] Saved Cox_S4_Drop1st.csv (", nrow(sens4_results_df), " rows)\n")
  } else {
    cat("  [WARN] No S4 results to save\n")
  }
}, error = function(e) {
  cat("  [ERROR] Failed to save S4 results:", e$message, "\n")
})
...
# Helper function to standardize P_value column to character
standardize_pvalue <- function(df) {
  if (is.null(df) || nrow(df) == 0) return(df)
  if ("P_value" %in% names(df)) {
    df$P_value <- as.character(df$P_value)
  }
  if ("P_Interaction" %in% names(df)) {
    df$P_Interaction <- as.character(df$P_Interaction)
  }
  return(df)
}

# Combine all results (including subgroup analyses)
# First standardize P_value columns to character type
all_results <- bind_rows(
  standardize_pvalue(individual_results_df) %>% mutate(Analysis = "Primary: Individual Factors"),
  standardize_pvalue(cumulative_results_df) %>% mutate(Analysis = "Primary: Cumulative (4-level)"),
  standardize_pvalue(sens1_results_df) %>% mutate(Analysis = "S1: 5-level Categories"),
  standardize_pvalue(sens2a_results_df) %>% mutate(Analysis = "S2a: Heavy Drink - Individual"),
  standardize_pvalue(sens2b_results_df) %>% mutate(Analysis = "S2b: Heavy Drink - Cumulative"),
  standardize_pvalue(sens3_individual_df) %>% mutate(Analysis = "S3: MICE - Individual"),
  standardize_pvalue(sens3_cumulative_df) %>% mutate(Analysis = "S3: MICE - Cumulative"),
  standardize_pvalue(sens4_results_df) %>% mutate(Analysis = "S4: Drop1st"),
  # Include subgroup analyses in all_results (also standardized)
  if(exists("age_stratified_df") && nrow(age_stratified_df) > 0) standardize_pvalue(age_stratified_df) else NULL,
  if(exists("sex_stratified_df") && nrow(sex_stratified_df) > 0) standardize_pvalue(sex_stratified_df) else NULL,
  if(exists("interaction_df") && nrow(interaction_df) > 0) standardize_pvalue(interaction_df) else NULL
)
