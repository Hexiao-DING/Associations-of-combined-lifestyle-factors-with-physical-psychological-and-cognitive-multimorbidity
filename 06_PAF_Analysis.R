...
if (nrow(individual_paf_df) > 0) {
  save_to_csv(individual_paf_df, "PAF_Primary_Individual.csv")
  cat("  Saved: PAF_Primary_Individual.csv (", nrow(individual_paf_df), " rows)\n")
}

# 2. Primary Cumulative PAF
if (nrow(cumulative_paf_df) > 0) {
  save_to_csv(cumulative_paf_df, "PAF_Primary_Cumulative.csv")
  cat("  Saved: PAF_Primary_Cumulative.csv (", nrow(cumulative_paf_df), " rows)\n")
}

# 3. Sensitivity S2 Heavy Drink
if (nrow(sens2a_df) > 0) {
  save_to_csv(sens2a_df, "PAF_S2_HeavyDrink_Individual.csv")
  cat("  Saved: PAF_S2_HeavyDrink_Individual.csv (", nrow(sens2a_df), " rows)\n")
}
if (nrow(sens2b_df) > 0) {
  save_to_csv(sens2b_df, "PAF_S2_HeavyDrink_Cumulative.csv")
  cat("  Saved: PAF_S2_HeavyDrink_Cumulative.csv (", nrow(sens2b_df), " rows)\n")
}

# 4. Sensitivity S3 MICE
if (nrow(sens3_individual_df) > 0) {
  save_to_csv(sens3_individual_df, "PAF_S3_MICE_Individual.csv")
}
if (nrow(sens3_cumulative_df) > 0) {
  save_to_csv(sens3_cumulative_df, "PAF_S3_MICE_Cumulative.csv")
}

# 5. Sensitivity S4 Drop1st
if (nrow(sens4_df) > 0) {
  save_to_csv(sens4_df, "PAF_S4_Drop1st.csv")
  cat("  Saved: PAF_S4_Drop1st.csv (", nrow(sens4_df), " rows)\n")
}
...
if (nrow(paf_summary) > 0) {
  save_to_csv(paf_summary, "PAF_Summary_Overall.csv")
  cat("  Saved: PAF_Summary_Overall.csv\n")
}
